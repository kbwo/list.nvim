...
-- Open new line below with list continuation (for 'o' in normal mode)
M.open_below_with_list = function()
  local cursor = vim.api.nvim_win_get_cursor(0)
  local line_num = cursor[1]
  local line = vim.api.nvim_buf_get_lines(0, line_num - 1, line_num, false)[1]
  ...
  if not list_info then
    -- Not a list, use default 'o' behavior
    vim.cmd("normal! o")
    return
  end
  ...
  vim.api.nvim_buf_set_lines(0, line_num, line_num, false, { new_line })
  -- Move cursor to the new line at the end of the prefix
  vim.api.nvim_win_set_cursor(0, { line_num + 1, #new_line })
  -- Enter insert mode
  vim.cmd("startinsert!")
end

-- Open new line above with list continuation (for 'O' in normal mode)
M.open_above_with_list = function()
  local cursor = vim.api.nvim_win_get_cursor(0)
  local line_num = cursor[1]
  local line = vim.api.nvim_buf_get_lines(0, line_num - 1, line_num, false)[1]
  ...
  if list_info.type == "checkbox" then
    new_line = indent_str .. list_info.marker .. " [ ] "
  elseif list_info.type == "ordered" then
    -- For opening above, use the same number (user can adjust numbering manually)
    new_line = indent_str .. tostring(list_info.number) .. ". "
  elseif list_info.type == "bullet" then
    new_line = indent_str .. list_info.marker .. " "
  end
